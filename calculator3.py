...
#                self.display = "Error"
        ...
        if button_name.isdigit():
            # falta A -> E if B==16     # aqui es para update
...
class CalculatorApp(App):
    ...
    async def on_load(self) -> None:
        await self.bind("g", "selectbase(2)", " BINARIO ")
        await self.bind("h", "selectbase(8)", " OCTAL ")
        await self.bind("j", "selectbase(10)", " DECIMAL ")
        await self.bind("k", "selectbase(16)", "HEXADECIMAL")
    ...
    def action_selectbase(self, b) -> None:
        act_style = "green on rgb(211,212,18)"
        dact_style = "yellow on rgb(20,40,20)"
        basedict = {10: 'DEC', 2: 'BIN', 8: 'OCT', 16: 'HEX'}

        if self.calc.baseT == b:
            return None

        print(self.calc.textos[basedict[self.calc.baseT]],
              ' | ', basedict[self.calc.baseT])
        self.calc.bases[self.calc.baseT] = False
        self.calc.textos[basedict[self.calc.baseT]].style = dact_style
        self.calc.bases[b] = True
        self.calc.textos[basedict[b]].style = act_style
        self.calc.baseT = b
    ...
    def on_key(self, event):
        if event.key.isdigit():
            ...
            self.calc.numbers[0].value = self.calc.numbers[0].value.lstrip(
                "0") + event.key
